...
// ─── TTS (streaming) ──────────────────────────────────────────────
/**
 * Streams TTS from ElevenLabs. Returns a Node Readable that yields Ogg-Opus
 * bytes as they're generated by the model — Discord starts playing as soon
 * as enough buffer accumulates, instead of waiting for the entire utterance
 * to render.
 *
 * Compared to the non-streaming endpoint this saves ~600–1000ms of
 * time-to-first-byte. The `optimize_streaming_latency=4` query param is an
 * ElevenLabs-specific knob that further trims TTFB at a small quality cost
 * (acceptable for short Discord roasts).
 *
 * Cache is bypassed in streaming mode — caching a stream cleanly is
 * nontrivial and the latency win matters more than the cache hit rate
 * for our use case (mostly unique adaptive quips anyway).
 */
async function fetchTTS(text) {
    ...
    if (!res.body) throw new Error('ElevenLabs returned empty stream body');

    // Convert the Web ReadableStream from fetch into a Node Readable
    // so @discordjs/voice can pipe it.
    return Readable.fromWeb(res.body);
}

function makeAudioResource(stream) {
...
async function playOnConnection(connection, guildId, text) {
    ...
    try {
        ...
        player.play(makeAudioResource(stream));
...
async function playInVoice(member, text, guildId) {
    ...
    try {
        ...
        const [stream, entry] = await Promise.all([
            fetchTTS(text),
            getOrJoinConnection(voiceChannel, guildId),
        ]);

        console.log(`[Voice] Ready. Streaming audio. Playing.`);

        entry.player.play(makeAudioResource(stream));
